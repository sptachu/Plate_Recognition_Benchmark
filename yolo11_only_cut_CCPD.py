...
# --- CCPD DICTIONARIES (Kept same as before) ---
...
def wytnij_i_zapisz():
    ...
    for split_name, txt_file in SPLIT_FILES.items():
        ...
        # We read the whole list, shuffle it, and take the first N
                    ...
                    for result in results:
                        for box in result.boxes:
                            ...
                            if oblicz_iou([px1, py1, px2, py2], gt_box) >= 0.5:
                                szer, wys = px2 - px1, py2 - py1
                                nx1 = px1
                                nx2 = px2
                                ny1 = py1
                                ny2 = py2

                                plate_crop = img[max(0, ny1):min(h_img, ny2), max(0, nx1):min(w_img, nx2)]

                                if plate_crop.size > 0:
                                    crop_name = f"{split_name}_crop_{saved_crops_count:06d}.jpg"
                                    cv2.imwrite(os.path.join(output_dir, crop_name), plate_crop)
                                    labels_file.write(f"{crop_name}\t{true_txt}\n")
                                    saved_crops_count += 1
                                    break # Only one plate per image in CCPD
