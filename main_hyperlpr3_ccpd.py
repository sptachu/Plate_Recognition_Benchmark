...
# --- INICJALIZACJA ---
print("[*] Inicjalizacja HyperLPR3 (CCPD Mode)...")
# Zmieniamy detect_level na HIGH, żeby zminimalizować FN (gubienie tablic)
...
# --- RAPORT KOŃCOWY ---
if processed_images > 0:
    # Wyliczanie pełnych metryk detekcji
    Precision = TP / (TP + FP) if (TP + FP) > 0 else 0.0
    Recall = TP / (TP + FN) if (TP + FN) > 0 else 0.0
    F1 = 2 * (Precision * Recall) / (Precision + Recall) if (Precision + Recall) > 0 else 0.0

    # Metryki OCR
    acc = (exact_matches / TP * 100) if TP > 0 else 0.0
    avg_cer = (total_cer / TP) if TP > 0 else 1.0
    ...
    print(f"Latency  -> Średnio E2E: {avg_ms:.2f} ms")
    print("=" * 50)

    # --- ZAPIS PEŁNEGO RAPORTU ZGODNEGO Z SHOW_RESULTS.PY ---
    ...
    with open(os.path.join(RESULTS_DIR, filename_res), 'w', encoding='utf-8') as f:
        ...
        # Wypisujemy czasy cząstkowe podzielone na pół, aby zaspokoić show_results.py
